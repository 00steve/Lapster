...
void GPS::startSerial(const uint16_t baud) {
    ...
    setUpdateRate(UPDATE_RATE_1000);
    sendCommand(PMTK_SET_BAUD_9600,true,500);
*/
    gpsSwSerial->begin(9600);
    sendCommand(PMTK_SET_BAUD_57600,true,500);
    gpsSwSerial->flush();
    gpsSwSerial->begin(57600);
    setUpdateRate(UPDATE_RATE_100);
...
double GPS::degreesAndFractionalMinutesToDegrees(double deg, const char hemi)
{
    /*
    const double degPart = (int)(deg/100.0);
    deg = (double)(deg - degPart* 100.0) / (double)60.0 + degPart;
    if (hemi == 'S' || hemi == 'W') {
        return -deg;
    } else {
        return deg;
...
uint8_t GPS::parseGGA(const char* nmea, const uint32_t now)
{
    ...
    latitudeDegreesAndFractionalMinutes = latitude;
...
String GPS::StatusString(){
    return "gps: " + String(latitude) + "," + String(longitude) + "," + String(altitude) + "\n";
}
void GPS::Update(){
...
Double3 GPS::Position(){ return Double3(longitude, altitude, latitude); }



unsigned char GPS::ColumnCount(){
    return 0;
};

String GPS::ColumnLabel(unsigned char index){
    return "";
};

unsigned char GPS::ColumnSize(unsigned char index){
    return 0;
};
